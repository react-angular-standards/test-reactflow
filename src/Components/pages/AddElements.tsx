...
import { TagIcon, WorkflowIcon, PeopleIcon, CalendarIcon, DownloadIcon } from "@primer/octicons-react";
...
export const AddElements = (): JSX.Element => {
  ...
  const [selectedRequirementObject, setSelectedRequirementObject] = React.useState<any>({});
  ...
  const text_area_change_event = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    fetch(UrlConstant.MANAGE_ASSOCIATE + "Template" + templateObject.id, {
      ...
      .then((result) => {
        ...
        if (id != undefined) {
          ...
          fetch(UrlConstant.QUERY_TEMPLATE_BY_ID + templateObject["id"]?.toString(), {
            mode: "cors",
            credentials: "include",
          })
            .then((res) => res.json())
  ...
  return (
    <div>
      <Breadcrumb aria-label="Large breadcrumb example with buttons"
        size="small"
        ...
        <BreadcrumbDivider />
        <BreadcrumbItem>
          <BreadcrumbButton onClick={() => (window.location.href = "#/formbuilder")} >
            ...
            <div className="col-md-9">
              <div className="row">
                <div className="col-md-12">
                  <span style={{ textDecoration: "none", marginBottom: 10, display: "block" }}>
                ...
                <div className="col-md-12">
                  <Label htmlFor={"outlineId"} style={{ marginTop: 20, marginBottom: 10 }}>
                    <strong>Description</strong>
                  </Label>
                  <br />
                  <SkeletonItem size={72} />
                </div>
                <div className="col-md-2" style={{ marginTop: 20, marginBottom: 20 }}>
              ...
              <div className="row">
                <div className="col-md-12" style={{ marginBottom: 30 }}>
                  {templateObject.isDeleted && (
                    <MessageBar style={{ marginTop: 20, color: "red", padding: 10 }} intent={"error"}>
                      <MessageBarBody style={{ fontSize: 14 }}>
                        Template <MessageBarTitle style={{ fontSize: 14 }}>{templateObject.header}</MessageBarTitle> is deleted, and
                        cannot be edited or used in any new statement of work.
                      </MessageBarBody>
                    </MessageBar>
                  )}
                </div>
                <div className="col-md-12">
                  ...
                  <Input disabled={templateObject.isDeleted} style={{ width: "100%" }} onChange={handletextchange} appearance="filled-darker" name="header" value={templateObject.header} />
                </div>
                <div className="col-md-12">
                  <Label htmlFor={"outlineId"} style={{ marginTop: 20, marginBottom: 10 }}>
                    <strong>Description</strong>
                  </Label>
                  <Textarea disabled={templateObject.isDeleted} style={{ width: "100%" }}
                ...
                <div className="col-md-12">
                  {disableSave && (
                    <MessageBar style={{ marginTop: 20 }} intent={"success"}>
                      <MessageBarBody>
                        Template <MessageBarTitle>{templateObject.header}</MessageBarTitle> created successfully! ,
                ...
                <div className="col-md-12">
                  <div style={{ overflowY: "auto", maxHeight: "calc(100vh - 100px)", marginTop: 20, background: "#f0f0f0", padding: 20 }}>
                        ...
                        return (
                          ...
                          >
                            ...
                            <Button
                              ...
                              styles={{
                                root: {
                                  borderColor: isTemplate ? 'green' : 'red', // Change border color based on condition
                            ...
                            {/* <Button disabled={templateObject.isDeleted} size="small" onClick={() => associateRequirements(node)}>
                              {(node.order !== undefined ? node.order + ". " : "") + node.header}
                            </Button> */}
                          </Tooltip>
                        );
            ...
            <div className="col-md-3">
              <div className="row">
                <div className="col-md-12">
                  ...
                  <Select disabled={templateObject.isDeleted} value={templateObject.status} appearance="filled-darker" name="status">
                  ...
                  {templateObject.Updated_by != undefined &&
                    <>
                      ...
                      <span style={{ display: "block", marginLeft: "20px" }}>{templateObject.Updated_by["NAME"]}</span>
                      <span
                        style={{
                          textDecoration: "none",
                          display: "block",
                      ...
                      <span style={{ display: "block", marginLeft: "20px" }}>
                        {new Date(templateObject.UPDATED_ON ? templateObject.UPDATED_ON : "").toString()}
                      </span>
                    </>
                  }

                  <Field style={{ maxWidth: 400, marginTop: 20 }}>
                    <span style={{ textDecoration: "none", display: "block", marginBottom: 10 }}>
                      <TagIcon size={16} /> <strong>Tags</strong>
                    </span>
                    {selectcount % 2 == 0 ? (
                      <TagPicker disabled={templateObject.isDeleted}
                        size="medium"
                        appearance="filled-darker"
                      ...
                      >
                        <TagPickerControl>
                          <TagPickerGroup>
                            {templateObject.Tag?.map((option: any) => (
                              <Tag disabled={templateObject.isDeleted}
                                key={option.NAME}
                                shape="rounded"
                        ...
                        <TagPickerList>
                          {tags.length > 0
                            ? tags.map((option: any) => (
                              <TagPickerOption
                                media={<Avatar shape="square" aria-hidden name={option.NAME} color="colorful" />}
                                value={option}
                                key={option.NAME}
                              >
                                {option.NAME}
          ...
          <DialogContentText id="alert-dialog-description">
            ...
            <Autocomplete
              ...
              getOptionLabel={(option: any) =>
                option.hasInput ? `${option.header} - (Object) ` : `${option.header} - (Template)`
              }

              defaultValue={selectedRequirementObject["children"]}
              renderInput={(params) => <TextField {...params} label={"Requirements Object"} />}
